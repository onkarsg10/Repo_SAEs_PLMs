...
class TopKAuto(nn.Module):
    ...
    def _decode(self, mid_sparse, target_mean=None, target_std=None):
        reconstruction = torch.matmul(mid_sparse, self.weights[1]) + self.weights[3]
    ...
    def forward(self, x, y):
        ...
        # Preprocess input
        shifted_input, _, _ = self._preprocess_input(x)
        # Encode
        ...
        reconstruction = self._decode(mid_sparse)
        ...
        main_loss = torch.mean((y - reconstruction) ** 2) #default loss used for both our LitLit steps 
        ...
        if torch.count_nonzero(inactive_mask).item() != 0 and self.training:
            aux_reconstruction = self._process_inactive_neurons(
                mid_before_sparse, inactive_mask
            )
    ...
    def _process_inactive_neurons(self, latents, inactive_mask):
        ...
        return self._decode(inactive_activated_new)
...
class LitLit(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        current_embeddings, next_embeddings, metadata, sequences = batch
        
        outputs = self(current_embeddings, next_embeddings)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        main_loss = self(current_embeddings, next_embeddings)
