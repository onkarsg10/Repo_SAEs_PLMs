...
def collate_fn(batch):
    ...
    max_len = max([emb.size(0) for emb in current_embeddings])
    embed_dim = current_embeddings[0].size(1)
    
    # Create padded tensors for both current and next embeddings
    batch_size = len(current_embeddings)
    padded_current = torch.zeros(batch_size, max_len, embed_dim, device=current_embeddings[0].device)
    padded_next = torch.zeros(batch_size, max_len, embed_dim, device=next_embeddings[0].device)
    
    # Fill the padded tensors
    for i, (curr_emb, next_emb) in enumerate(zip(current_embeddings, next_embeddings)):
        padded_current[i, :curr_emb.size(0)] = curr_emb
        padded_next[i, :next_emb.size(0)] = next_emb
    
    return (padded_current, padded_next, 
