...
def collate_fn(batch):
    # Unzip the batch into embeddings, metadata, and sequences
    embeddings, metadata, sequences = zip(*batch)
    
    # Concatenate all embeddings along dimension 0
    # This creates a tensor of shape [total_amino_acids, D]
    concatenated_embeddings = torch.cat(embeddings, dim=0)
    
    return concatenated_embeddings, list(metadata), list(sequences)
...
class dmod(pl.LightningDataModule):
    def __init__(self, uniref_file, esm_model, alphabet, device, esm_layer, max_seq_len, 
                 ...
                 max_samples=50000000, num_workers=0):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        dataset = UniRefDataset(
            ...
            self.esm_layer, self.max_seq_len, self.seed_only, self.max_samples
        )
