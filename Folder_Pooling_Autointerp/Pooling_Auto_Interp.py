...
def find_pooling_files(folder_path):
    ...
    try:
        ...
        embeddings_files = list(folder.glob("POOLING_embeddings*"))
        activations_files = list(folder.glob("POOLING_activations*"))
        indices_files = list(folder.glob("POOLING_indices*"))
        metadata_files = list(folder.glob("POOLING_metadata*"))
        
        # Check if exactly one file was found for each prefix
        for file_type, files in [
            ("embeddings", embeddings_files),
            ("activations", activations_files),
            ("indices", indices_files),
            ("metadata", metadata_files)
        ]:
            if len(files) == 0:
                print(f"Error: No POOLING_{file_type} file found in {folder_path}")
                return None
            if len(files) > 1:
                print(f"Error: Multiple POOLING_{file_type} files found in {folder_path}:")
                for f in files:
                    print(f"  - {f.name}")
                return None
        ...
        print(f"Activations: {activations_files[0].name}")
        print(f"Indices: {indices_files[0].name}")
        print(f"Metadata: {metadata_files[0].name}\n")
        
        return files
        
    except Exception as e:
        print(f"Error while searching for POOLING files: {str(e)}")
        return None
...
class NeuronInterpreter:
    def __init__(self, embeddings_file, activations_file, indices_file, metadata_file, 
        ...
        with open(self.results_file, 'w') as f:
            ...
            f.write(f"activations_file: {activations_file}\n")
            f.write(f"indices_file: {indices_file}\n")
        ...
        self.load_data(embeddings_file, activations_file, indices_file, metadata_file)


        # Main data loading method that handles both embedding and activation analysis
    # Input: File paths for embeddings, activations, indices, and metadata
    # Output: Processed data stored in class attributes (self.full_activation_matrix, etc.)
    def load_data(self, embeddings_file, activations_file, indices_file, metadata_file):

        # Load raw embedding vectors from numpy file into memory as a dense matrix
        self.embeddings = np.load(embeddings_file)  # Load embedding vectors
        # Load activation values which represent the strength of neuron firings
        self.activations = np.load(activations_file)  # Load activation values
        # Load indices that map which neurons were activated (for sparse representation)
        self.indices = np.load(indices_file)  # Load activation indices
        # Load sequence metadata containing additional information about each sequence
        self.metadata = pd.read_csv(metadata_file)  # Load sequence metadata
        ...
        if hasattr(self, 'embedding_mode') and self.embedding_mode:
            ...
            # Extract dimensions from embedding matrix shape
            self.num_sequences = self.embeddings.shape[0]  # Number of sequences
            self.num_neurons = self.embeddings.shape[1]  # Number of dimensions/neurons
            ...
            # Print debug information about the resulting activation matrix
            print(f"DEBUG: Full activation matrix shape: {self.full_activation_matrix.shape}")
            # Print the range of values after normalization (should be 0 to 1)
            print(f"DEBUG: Activation range after scaling: [{self.full_activation_matrix.min():.3f}, {self.full_activation_matrix.max():.3f}]")
        
        # Branch for handling sparse activation mode
        else:
            # Print debug information about activations and shapes
            print(f"DEBUG: Activations shape/type: {np.shape(self.activations)}")
            print(f"DEBUG: Activations shape/type (without np.shape): {self.activations.shape}")
            print(f"DEBUG: Length of activations list: {len(self.activations)}")
            print(f"DEBUG: Indices shape/type (without np.shape): {self.indices.shape}")
            print(f"DEBUG: Length of indices list: {len(self.indices)}")
            # Add check for min/max indices
            print(f"DEBUG: Min index in self.indices: {self.indices.min()}, Max index: {self.indices.max()}")
            
            # Set dimensions for constructing the dense activation matrix
            self.num_sequences = len(self.activations)  # Number of sequences
            self.num_neurons = 20000  # Fixed number of neurons (hardcoded)
            # Create empty matrix to store dense representation of activations
            self.full_activation_matrix = np.zeros((self.num_sequences, self.num_neurons))
            
            # Print initial shape of full activation matrix
            print(f"DEBUG: Full activation matrix initial shape: {self.full_activation_matrix.shape}")
            
            # Convert sparse representation to dense matrix
            for seq_idx in range(self.num_sequences):
                # Fill in non-zero activations at their corresponding indices
                self.full_activation_matrix[seq_idx, self.indices[seq_idx]] = self.activations[seq_idx]
            
            # Print shape after filling in values
            print(f"DEBUG: Full activation matrix final shape: {self.full_activation_matrix.shape}")
            
            # Begin normalization process for sparse activation matrix
            # Calculate minimum value for each neuron across all sequences
    ...
    def get_sequences_for_neuron(self, neuron_idx, high_block_samples=6, mid_block_samples=3, 
        ...
        def sample_from_block_with_spillover(acts, block_idx, target_size, used_indices=None):
            """
            Enhanced spillover logic that tries current block first, then alternates between lower and higher blocks.
            """
    ...
    def format_sequence_data(self, seq_idx, activation=None):
        ...
        def safe_get(column_name):
            if column_name in seq_data.index:
                return str(seq_data[column_name])
            return ''
        
        # Build formatted dictionary with conditional sequence inclusion
        formatted_data = {
        ...
        # Only include sequence if specified
        if self.include_sequence:
            formatted_data['sequence'] = safe_get('sequence')
    ...
    def get_claude_interpretation(self, train_data):
        ...
        # Construct the initial prompt with neuron information
...
"""
        # Add detailed information for each active example sequence
        for seq in train_data['positive']:
            prompt += f"\nSequence ID: {seq['sequence_id']}"
            prompt += f"\nActivation: {seq['activation']:.10f}"
            prompt += f"\nProtein name: {seq['protein_names']}"
            prompt += f"\nGene name: {seq['gene_names']}"
            prompt += f"\nProtein families: {', '.join(seq['protein_families'])}"
            prompt += f"\nBiological process (GO): {', '.join(seq['go_biological'])}"
            prompt += f"\nCellular component (GO): {', '.join(seq['go_cellular'])}"
            prompt += f"\nMolecular function (GO): {', '.join(seq['go_molecular'])}"
            if self.include_sequence:
        ...
        # Add detailed information for each inactive example sequence
        ...
        # Add final instruction for Claude's interpretation
    ...
    def get_claude_predictions(self, interpretation, test_data, neuron_idx):
        ...
        # Construct the test data portion of the prompt
        test_prompt = ""
        ...
        # Construct the complete prompt with the cleaned interpretation
...
{test_prompt}"""
        ...
        if self.offline_mode:
            ...
            print("After adding interpretation to the file, paste it to Claude.")
    ...
    def analyze_neuron(self, neuron_idx, high_block_samples=6, mid_block_samples=3, 
            ...
            train_data = {
                ...
                'positive': [self.format_sequence_data(idx, act) 
                            for idx, act in zip(train_active_indices, train_active_values)],
                'negative': [self.format_sequence_data(idx, self.full_activation_matrix[idx, neuron_idx]) 
            ...
            # Each sequence is formatted using format_sequence_data helper method
            test_data = (
                [self.format_sequence_data(idx, act) 
                for idx, act in zip(test_active_indices, test_active_values)] +
                [self.format_sequence_data(idx, self.full_activation_matrix[idx, neuron_idx]) 
...
def main():
    ...
    embeddings_file, activations_file, indices_file, metadata_file = files
